    ...
    const renderGrid = (cards: DeckCard[]) => {
        return (
            <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 8 }}>
                {cards.map((item) => (
                    <View key={item.card._id} style={{ width: 80, alignItems: 'center' }}>
                        <Image
                            source={{ uri: `https://s3.duellinksmeta.com/cards/${item.card._id}_w420.webp` }}
                            style={{ width: 70, height: 100, borderRadius: 4, marginBottom: 4 }}
                        />
                        <Text style={{ color: 'white', fontSize: 12, textAlign: 'center' }}>x{item.amount}</Text>
                    </View>
                ))}
            </View>
        )
